...
var temp = 0;
var traits = []; //array that stores the stdDEV
var count = 1;
//NEW DECLARATIONS
...
app.post('/tr-results', (req, res) => {
    ...
    popSongData.forEach(a => a.splice(2,1));
    popSongData.forEach(a => a.splice(3,1));
    popSongData.forEach(a => a.splice(9,1));

    //get the sum
    for (var i =0; i < popSongData.length; i++){
        if(popSongData[i] != null){
            for (var j = 0; j < popSongData[i].length;j++){
                traits.push(parseFloat(popSongData[i][j]));
            }
        }
        break;
    }

    for (var i = temp + 1; i < popSongData.length; i++){
        if(popSongData[i] != null){
            for (var j = 0; j < popSongData[i].length;j++){
                traits[j] += (parseFloat(popSongData[i][j]));
            }
            count++;
        }
    }
    ...
    for (var i = 0; i < popSongData.length;i++){ //each row
        if(popSongData[i] != null){
            scoreDeviation.push([]);
            for (var j = 0; j < popSongData[i].length;j++){
                scoreDeviation[i].push((popSongData[i][j] - meanArray[j]) ** 2);
            }
        }
    }
