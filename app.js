...
app.post('/as-search', (req, res) => {
    ...
    if (isSong) {
        ...
        res.render('as-search', {
            isSong_ejs: false,
            artistSearched: artists
        });
    }
});

app.post('/artist-similarity', (req, res) => {
    var selectedArtist = req.body.artist;
    var artistToDelete = req.body.artistToDelete;
    var deleteIndex = user_artists.indexOf(artistToDelete);

    if (selectedArtist != undefined) {
        user_artists.push(selectedArtist);
        console.log(user_artists)
    }

    console.log(deleteIndex);
    if (deleteIndex > -1 && artistToDelete != null) {
        user_artists.splice(deleteIndex, 1);
        console.log(user_artists)
    }


    res.render('artist-similarity', {
        selectArtists: user_artists,
    });
});
...
app.post('/as-results', (req, res) => {
    ...
    for (var i = 0; i < user_artists.length; i++) {
        ...
        topThreeSongs.push(getTopThree(a_songs));
        a_songs = [];
    }
...
function getTopThree(songs) {
    //Convert string integer to integer
    for (var i = 0; i < songs.length; i++) {
        songs[i][2] = parseInt(songs[i][2])
    }

    //Return if 3 songs or less
    if (songs.length <= 3) {
